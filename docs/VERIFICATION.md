...
sitting against the **live production deployment**. Nothing is summarised, compressed, or
retyped — if a line is not in the output, it is not in this document.

**Live:** https://groundtruth-hedera.vercel.app
**Network:** `hedera:testnet` · **Asset:** Circle USDC (`0.0.429274`) · **Facilitator:** `https://api.testnet.blocky402.com` · **Database:** MongoDB Atlas
...
Point the last two at production with `NEXT_PUBLIC_APP_URL=https://groundtruth-hedera.vercel.app`.

**243 assertions passing, 0 failing.**
...
Only the offline suite runs in CI — on every push to `main` and every pull request. The rest need
funded keys, a live database and a running server, and spend real USDC.
...
The CJS build of Vite's Node API is deprecated. See https://vite.dev/guide/troubleshooting.html#vite-cjs-node-api-deprecated for more details.
 ...
 ✓ test/money.test.ts (13 tests) 21ms
 ✓ test/types.test.ts (18 tests) 7ms
 ✓ test/api-human-do.test.ts (18 tests) 19ms
 ✓ test/notary.test.ts (18 tests) 107ms
 ✓ test/api-lifecycle.test.ts (16 tests) 88ms
 ✓ test/verify.test.ts (9 tests) 202ms
 ✓ test/x402.test.ts (12 tests) 352ms
 ✓ test/hedera.test.ts (17 tests) 373ms
 ✓ test/mirror-verify.test.ts (11 tests) 382ms

 Test Files  9 passed (9)
      Tests  132 passed (132)
   Start at  00:37:26
   Duration  857ms (transform 512ms, setup 0ms, collect 741ms, tests 1.55s, environment 1ms, prepare 643ms)
```
...
3. Agent signs a Hedera transfer for the challenge
  ✓ payment header produced
      header is 3452 bytes of base64
...
6. Facilitator settles on Hedera
  ✓ settled — tx 0.0.7162784@1785524860.300128020
      https://hashscan.io/testnet/transaction/0.0.7162784@1785524860.300128020

7. Independent confirmation on a public Mirror Node
  ✓ confirmed in consensus at 1785524866.126201388
      payer  0.0.9847870
      payee  0.0.9847867
      amount 0.5 USDC

8. Negative — replaying the settled payment must fail
  ✓ replay rejected — "transaction 0.0.7162784@1785524860.300128020 failed precheck with status DUPLICATE_TRANSACTION against node account id 0.0.7"

9. Confirm value actually moved
      agent  16.66 → 16.16  (-0.5)
      payTo  16.58 → 17.08  (0.5)
  ✓ payee credited 0.5 USDC
  ✓ agent debited exactly the price — facilitator covered the network fee

  HashScan: https://hashscan.io/testnet/transaction/0.0.7162784@1785524860.300128020
...
- **The agent is debited exactly the price — no gas on the payment itself.** The transaction id
  ...
  payment asset and a little HBAR for the one-time token association, nothing more.
- **Replay is refused before it reaches our database.** The signed transaction carries its own
  ...
  Past that window the unique index on `payments.tx_hash` is what stops it.
...
3. Native payout to an oracle account
      oracle 0.0.9847870 before: 16.16 USDC
  ✓ payout settled — 0.0.9847867@1785524873.561560316
      https://hashscan.io/testnet/transaction/0.0.9847867@1785524873.561560316
  ✓ oracle credited exactly 0.44 USDC

4. Anchor a proof to Hedera Consensus Service
  ✓ anchored at sequence #21 — 0.0.9847867@1785524875.795532387
      https://hashscan.io/testnet/transaction/0.0.9847867@1785524875.795532387

5. Read the anchor back from a public Mirror Node
  ✓ anchor readable from public mirror at consensus 1785524883.846834104
...
2. Task round trip
  ✓ inserted task 55ec21d9-5996-4611-9772-ec394b728b92
...
6. Proof storage (GridFS)
  ✓ uploaded proof: 55ec21d9-5996-4611-9772-ec394b728b92/0-c414cd0e.png
...
7. Signed proof URLs
  ✓ signed URL minted
      https://groundtruth-hedera.vercel.app/api/proofs/55ec21d9-5996-4611-9772-ec394b728b92/0-c414cd0e.png?exp=1785528457&sig=55afd58822a8eb1789529daf22bf72c87c90405fa900d17e579e3e4c47255bb4
...
8. Worker reputation
  ✓ counters accumulate (2 completed)
...
are database constraints — a conditional `findOneAndUpdate` and a unique partial index — not
application-level checks, which would race.

---

## `pnpm test:endpoints` — every HTTP route, against production

```
Endpoint suite  https://groundtruth-hedera.vercel.app
...
Paid task creation (real USDC)
  ...
  ✓ task created 693c049b-6cc6-41ec-b7ae-c85c43616664
      payment tx 0.0.7162784@1785524889.941419943
...
Review and payout
  ...
  ✓ oracle paid — 0.44 USDC
      https://hashscan.io/testnet/transaction/0.0.9847867@1785524903.635922907
  ✓ proof anchored at HCS #22
...
## `pnpm e2e` — full lifecycle, against production

```
GroundTruth — full lifecycle on hedera:testnet
  server : https://groundtruth-hedera.vercel.app
  asset  : USDC (0.0.429274)
  budget : 0.50
  oracle : 0.0.9860142
...
[1] Agent fetches the 402 challenge and signs a Hedera payment
      ...
      Balance: 16.1 USDC (need 0.50)
      Requesting 402 challenge from https://groundtruth-hedera.vercel.app/api/v1/human-do...
...
[2] Paid request creates the task
  ✓ task fe2c3411-29a2-4d67-93d7-1899cc89899b created
      payment tx 0.0.7162784@1785524910.807306689
      https://hashscan.io/testnet/transaction/0.0.7162784@1785524910.807306689
...
[5] Oracle submits photo proof
      freshness code for this task: 6V7T4J
  ✓ submitted — status submitted
      notary: uncertain (vision error (network/timeout))
...
[7] Oracle is paid on Hedera and the proof is anchored to HCS
  ✓ payout tx 0.0.9847867@1785524943.299405326
      https://hashscan.io/testnet/transaction/0.0.9847867@1785524943.299405326
  ✓ paid 0.44 USDC
  ✓ proof anchored at HCS sequence #23
      https://hashscan.io/testnet/topic/0.0.9847942
      oracle balance 2.2 → 2.64 (0.44)
...
[8] Paying agent retrieves the proof it paid for
  ...
  HashScan
    payment : https://hashscan.io/testnet/transaction/0.0.7162784@1785524910.807306689
    payout  : https://hashscan.io/testnet/transaction/0.0.9847867@1785524943.299405326
    anchor  : https://hashscan.io/testnet/topic/0.0.9847942
    board   : https://groundtruth-hedera.vercel.app/tasks/fe2c3411-29a2-4d67-93d7-1899cc89899b

ALL PASS — 13 passed, 0 failed
```

**Payment:** [`0.0.7162784@1785524910.807306689`](https://hashscan.io/testnet/transaction/0.0.7162784@1785524910.807306689)
**Payout:** [`0.0.9847867@1785524943.299405326`](https://hashscan.io/testnet/transaction/0.0.9847867@1785524943.299405326)
...
an EVM alias. The child is often returned **first** with an empty transfer list, so reading
`transactions[0]` found no payment and reported every genuine payment as unconfirmed. Now scans
all successful records. The same fix corrected payer attribution: the first debit can be the fee
payer rather than the sender, so the payer is matched on the exact transfer amount.

**2. Decimals were hardcoded to 6.**
HBAR is quoted in tinybars at 8, so a task priced `"2.00"` would have charged **0.02 HBAR** — a
100× underpayment. `lib/money.ts` now derives its scale from the configured asset.

**3. ED25519 keys were parsed as ECDSA.**
`fromStringECDSA` does not throw on an ED25519 DER string — it derives a *different* public key,
so the account id is wrong and every signature fails for no visible reason. Hedera Portal issues
both types. For DER keys the curve is now read from the algorithm identifier.

**4. The task state machine was decorative.**
`VALID_TRANSITIONS` looked authoritative, but `transition()` never consulted it, so
`pending → verified` — paying out for work nobody did — was one call away. Now enforced.

**5. A 429 sent verifiable proofs to manual review.**
The vision client rotated providers on rate-limit, which does nothing with a single key. Now
...
named in the README that had never once fired.

**7. `payment_ref` leaked from a public endpoint.**
`GET /api/tasks` returned whole task documents including the credential that authorises a payout.
The route now projects an explicit allowlist and the credential is compared in constant time.

**8. A caller could set their own price.**
`budget_usdt` is caller-supplied and becomes the price the whole pipeline enforces, while the
schema constrained only its format. A caller could declare `0.000001`, pay it, and clear all
three gates legitimately. Now floored at the list price.

**9. Fixing (6) broke production.**
The first fix made *perceptual-hash equality* a hard fail and called it a "byte-for-byte repeat".
Those are not the same thing: an 8×8 average hash collides for images differing in a small region
— such as the same template with a different freshness code — so the second deployed photo
submission was rejected as a duplicate. Exact identity now uses a SHA-256 digest (hard fail) and
the perceptual hash is a similarity hint (advisory). `test/verify.test.ts` covers both directions.

**10. Every photo was judged twice.**
`lib/verify.ts` ran its own advisory vision check before `lib/notary.ts` ran the real one — same
image, same model, and the first call did not even pass the freshness code. On serverless, two
calls with retries could exceed the function budget on their own, which is how it surfaced: a
`vision error (network/timeout)` in production. The redundant call is gone.

Findings 6, 7 and 8 came from an adversarial audit of this repository's own documentation against
its code — nine judges, one per claim group, each told to refute rather than confirm. They
examined 132 claims and flagged 92. Findings 9 and 10 came from deploying and running the suite
against the deployment, which is why that step is not optional.
