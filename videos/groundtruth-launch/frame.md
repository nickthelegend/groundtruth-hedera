...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), Bricolage Grotesque for all
  display + Bricolage Grotesque body + JetBrains Mono for the index/code voice on a warm-navy code surface,
...
colors:
  # Lifted verbatim from the product's own dark theme (app/globals.css
  # [data-theme='dark']). The auto-inversion put the code surface on light grey
  # and pulled the verified-green into tiles; both are corrected here — this
  # video is dark end to end, and green is a STATE, not a surface.
  ink: "#EDF2F7"          # primary type
  ink-muted: "#7A9AB5"    # secondary type
  ink-faint: "#3A5269"    # tertiary, grid labels, axis ticks
  cream: "#04060A"        # canvas — the page ground
  tile: "#0C1420"         # elevated card / panel
  tile-strong: "#0A1119"  # inset / recessed well
  coral: "#0DCCFF"        # THE accent. Active / keyword / highlight ONLY.
  navy: "#0A1119"         # code surface
  navy-soft: "#0C1420"    # code surface, raised
  navy-elev: "#111C2B"    # code surface, highest
  verified: "#00E87A"     # state only — "verified" and "paid". Never decoration.
  line: "#1C2A3A"         # hairline
  grid-dot: "rgba(13,204,255,0.10)"  # the dot grid

borders: { hairline: "1px solid #1C2A3A", hairline-strong: "1px solid #274056", accent: "1px solid rgba(13,204,255,0.28)" }
shadows: { card: "none", accent-glow: "0 0 34px rgba(13,204,255,0.32)", none: "none" }
...
typography:
  ...
  # — display ramp (Bricolage Grotesque 700/800, sentence case, tight negative tracking — this is exactly how the product sets its own headlines) —
  headline:{ fontFamily: "Bricolage Grotesque", cqw: 4.6, weight: 800, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "Bricolage Grotesque", cqw: 5.0, weight: 700, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "Bricolage Grotesque", cqw: 6.7, weight: 700, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "Bricolage Grotesque", cqw: 7.3, weight: 800, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "Bricolage Grotesque", cqw: 9.4, weight: 800, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "Bricolage Grotesque", cqw: 9.9, weight: 800, lineHeight: 0.98, tracking: "-0.028em" }
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Bricolage Grotesque figure paired with a JetBrains Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (Bricolage Grotesque italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. Bricolage Grotesque italic, small Bricolage Grotesque uppercase cite beneath."
  section-rule:
...
- **Fonts** — already set to **Bricolage Grotesque** (display) / **JetBrains Mono** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 600, 700, 800}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
Three editorial voices, each in its own face: **Bricolage Grotesque** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **Bricolage Grotesque** carries body, leads, card titles, buttons, and
...
- **Bricolage Grotesque** (sentence case, negative-tracked) for all display; **Bricolage Grotesque** body/chrome; **JetBrains Mono** index + code.
...
- **Squint** — one Bricolage Grotesque display moment dominates at 3–6× its neighbor.
...
- **Type** — Bricolage Grotesque sentence-case display (negative-tracked); Bricolage Grotesque 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (Bricolage Grotesque `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (Bricolage Grotesque `headline` 4.6cqw
...
- **Bricolage Grotesque display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Bricolage Grotesque body** sentence case weight 400. **JetBrains Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — Bricolage Grotesque figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — Bricolage Grotesque italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized Bricolage Grotesque · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line Bricolage Grotesque `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** Bricolage Grotesque 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single Bricolage Grotesque line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line Bricolage Grotesque `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a Bricolage Grotesque `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Fixed** serif figure + mono unit, hairline rule. **Free** the figures (from the script), tag, layout + density. **Density** free.

### 5 · Pull-quote (quote · move: Bricolage Grotesque italic · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a Bricolage Grotesque **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small Bricolage Grotesque uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Fixed** Bricolage Grotesque italic quote + uppercase cite. **Free** quote, attribution, layout + density. **Density** free.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short Bricolage Grotesque sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships, layout + density. **Density** free.
...
- Set all display in **Bricolage Grotesque, sentence case**, negative-tracked; **Bricolage Grotesque 400** body; **JetBrains Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a Bricolage Grotesque figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case Bricolage Grotesque display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one Bricolage Grotesque display moment dominates.
...
- **Type** — Bricolage Grotesque sentence-case display (negative-tracked); Bricolage Grotesque 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
